...
def test_analyst_extracts_title(
    analyst_agent: AnalystAgent, sample_test_case_add_todo: str
) -> None:
    """The output must contain a non-empty title string."""
    result = analyst_agent.run({"test_case": sample_test_case_add_todo})
    assert "title" in result
    assert isinstance(result["title"], str)
    assert len(result["title"]) > 0


def test_analyst_extracts_steps(
    analyst_agent: AnalystAgent, sample_test_case_add_todo: str
) -> None:
...
def test_analyst_handles_search_test_case(
    analyst_agent: AnalystAgent, sample_test_case_complete_todo: str
) -> None:
    """Structural checks must hold for the complete-todo test case fixture too."""
    result = analyst_agent.run({"test_case": sample_test_case_complete_todo})
